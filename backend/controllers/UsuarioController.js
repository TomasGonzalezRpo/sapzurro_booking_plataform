...
const baseFindOptions = {
  include: [
    // mantenemos este include por compatibilidad, pero el adaptador es tolerante
 ...
 * para que el frontend reciba siempre: id_usuario, username, nombres, apellidos,
 * fullName, correo, id_perfil, rol, provider, estado
 */
function adaptUsuario(u) {
  if (!u) return null;

  // si viene una instancia de Sequelize, tomar dataValues para simplificar
  const raw = u.dataValues ? u.dataValues : u;

  // distintos posibles lugares donde puede aparecer la relación Persona
  const personaCandidates = [
    raw.personaInfo,
    raw.persona,
    raw.Persona,
    raw.Person, // por si acaso
    (raw.dataValues && raw.dataValues.personaInfo) || undefined,
  ].filter(Boolean);

  const persona = personaCandidates.length > 0 ? personaCandidates[0] : null;
  ...
  const rol = raw.perfil?.nombre ?? raw.rol ?? "Desconocido";
...
exports.createUsuario = async (req, res) => {
  ...
  try {
    ...
    // Recuperar usuario con relaciones y normalizar la salida
    const usuarioCreadoRaw = await Usuario.findByPk(nuevoUsuario.id_usuario, {
      include: [
        { model: Persona, as: "personaInfo" },
        { model: Perfil, as: "perfil" },
      ],
...
exports.updateUsuario = async (req, res) => {
  ...
  try {
    ...
    const usuarioActualizadoRaw = await Usuario.findByPk(id, {
      include: [
        { model: Persona, as: "personaInfo" },
        { model: Perfil, as: "perfil" },
      ],
